...
def analyze(item):
    ...
    return {**item,"date":last["date"],"close":last["close"],"pct":last["pct"],"ret20":fmt(change_pct(last["close"],closes[-21])),"ret60":fmt(change_pct(last["close"],closes[-61])),"ret120":fmt(change_pct(last["close"],closes[-121])) if len(closes)>120 else None,"ma":{k:fmt(x,3) for k,x in mas.items()},"bbi":daily["bbi"],"kdjJ":daily["j"],"bias55":fmt(change_pct(last["close"],mas["55"]),1),"volRatio":fmt(vol_ratio,2),"pos20":fmt(pos20,1),"hi20":fmt(hi20,3),"low20":fmt(low20,3),"status":status,"actionLevel":level,"heldAction":held_action,"unheldAction":unheld_action,"action":held_action,"reasons":reasons,"risks":risks or ["当前未触发材料中的突出卖出红线"],"nextWatch":next_watch,"checks":checks,"timeframes":{"month":monthly,"week":weekly,"day":daily},"n":n,"signals":{"crossUpBbi":cross_up_bbi,"crossDownBbi":cross_down_bbi,"crossDownMa60":cross_down_ma60,"stopCandle":stop_candle,"bullishConfirm":bullish_confirm,"highVolumeStall":high_volume_stall,"topBear":top_bear},"series":series}
